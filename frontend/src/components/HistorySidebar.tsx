      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className='fixed right-0 top-0 h-full w-full md:w-96 bg-gray-950/95 backdrop-blur-xl border-l border-white/10 z-[60] shadow-2xl p-6 overflow-y-auto'
          >
            ...
            <div className='space-y-4'>
              {history.length === 0 ? (
                ...
                history.map((meal) => (
                  <div
                    key={meal._id}
                    className='group bg-white/5 p-3 rounded-2xl flex items-center space-x-4 hover:bg-white/10 transition-colors cursor-pointer border border-white/5 hover:border-white/30'
                    onClick={() => {
                      onSelectMeal(meal);
                      onClose();
                    }}
                  >
                    ...
                    <div className='flex-1 min-w-0'>
                      <p className='font-bold truncate text-gray-100'>{meal.foodName}</p>
